// Login form using React Router Form component
import { useEffect } from 'react';
import { Form, useActionData, useNavigation, useNavigate, useOutletContext } from 'react-router-dom';
...
export function LoginForm() {
  ...
  const { setUser } = useOutletContext() as { user: any; setUser: (user: any) => void };
  
  // Check if we're submitting
  const isSubmitting = navigation.state === 'submitting';
  
  console.log('[LOGIN FORM] Component rendered');
  console.log('[LOGIN FORM] Navigation state:', navigation.state);
  console.log('[LOGIN FORM] Action data:', actionData);
    ...
    <div className="bg-white/80 backdrop-blur-sm p-8 md:p-10 rounded-2xl shadow-xl border border-gray-100">
      ...
      {actionData?.success && (
        <div className="bg-green-50 border border-green-200 text-green-700 px-4 py-3 rounded-xl mb-4">
          {actionData.message || 'Login successful!'}
        </div>
      )}
      ...
      <Form method="post" className="space-y-4">
        ...
        >
          {isSubmitting ? 'Signing in...' : actionData?.success ? 'Logged in!' : t('login.form.login')}
        </button>
      </Form>
