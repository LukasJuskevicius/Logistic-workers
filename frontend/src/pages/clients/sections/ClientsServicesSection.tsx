      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-8 md:mb-16">
          <div className="inline-flex items-center px-3 py-1.5 md:px-4 md:py-2 bg-white/80 backdrop-blur-sm rounded-full text-xs md:text-sm mb-4 md:mb-6">
            ...
            <span className="text-green-700 font-medium">{t('clients.services.badge')}</span>
          </div>
          ...
          <p className="text-sm sm:text-base md:text-lg lg:text-xl text-gray-600 max-w-3xl mx-auto px-2">{t('clients.services.subtitle')}</p>
        </div>
          ...
          {clientServices.map((service) => (
            <EnhancedCard key={service.id} variant="elevated" className="p-6 md:p-8">
              <div className="flex flex-col items-center text-center">
                ...
                {/* Content */}
                <h3 className="text-lg md:text-xl lg:text-2xl font-bold text-gray-900 mb-3 md:mb-4">{t(`clients.services.items.${service.id}.title`)}</h3>
                
                <p className="text-sm md:text-base text-gray-600 mb-4 md:mb-6">{t(`clients.services.items.${service.id}.description`)}</p>
                  ...
                  <ul className="space-y-2 text-sm md:text-base text-gray-700">
                    {service.features.map((feature, index) => (
                      <li key={index} className="flex items-center">
                        ...
                        {t(`clients.services.items.${service.id}.features.${index}`)}
                      </li>
                    ))}
                  </ul>
                </div>
                ...
                <div className="mt-auto">
                  <div className="inline-flex items-center px-3 py-1.5 bg-green-100 text-green-700 rounded-full text-xs md:text-sm font-medium">
                    ...
                    {t(`clients.services.items.${service.id}.timeFrame`)}
                  </div>
                </div>
              </div>
            </EnhancedCard>
